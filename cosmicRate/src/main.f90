...
if (CmdArg%count/=2) then
    ...
    write(output_unit,"(*(g0))") "       a.exe <input file path: ./in/> <input file name: WorldModelSimualtionGRB.nml>"
end if
...
blockErr: block
    ...
    use BatseSgrbWorldModel_mod, only: NPAR, getLogPostProb
    use BatseSgrbWorldModel_mod, only: zone_relerr, zgrb_relerr, liso_relerr, epkz_relerr
    use BatseSgrbWorldModel_mod, only: zone_neval, zgrb_neval, liso_neval, epkz_neval
    !use BatseSgrbWorldModel_mod, only: zgrb_count, liso_count, epkz_count
    type(Path_type) :: Path
