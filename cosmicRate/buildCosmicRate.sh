...
# This bash script file configures the flags required for building the GRB world model on non-Windows Operating-Systems.
...
system when ParaMonte is built via its provided build scripts at the root of its project's directory. 
    ...
    flag definitions:
        ...
        -z | --quadz            : The Romberg integration method for the redshit axis
...
BATSE_DATA_FILE_NAME="batseDataSGRB565.in"; export BATSE_DATA_FILE_NAME
echo >&2 "-- ${BUILD_NAME} - BATSE_DATA_FILE_NAME: ${BATSE_DATA_FILE_NAME}"

# GRB world model's SFR model: H06, L08, B10
...
if [ $? -eq 0 ]; then
    ...
    echo >&2 "-- ${BUILD_NAME} - from: ${CosmicRate_ROOT_PATH}/in/${BATSE_DATA_FILE_NAME}"
    echo >&2 "-- ${BUILD_NAME} -   to: ${CosmicRate_BIN_DIR}/in/"
