...
if (CmdArg%count/=3) then
    ...
    write(output_unit,"(*(g0))") "       a.exe <input file path: ../in/zestimation.nml> <sample file path: path to the cosmic rate posterior PDF>"
end if
...
call readDataGRB( inFilePath    = inputBatseDataFile    &
    ...
    rateModel = "F18"
#else
#error "Unknown SFR model in WorldModelForBatseSGRB_mod.f90"
#endif
