...
#error "Unknown SFR model in BatseSgrbWorldModel_mod.f90"
    ...
    real(RK)    , parameter :: zoneMin = 1.09_RK    ! 1.1e0_RK
    real(RK)    , parameter :: zoneMax = 7.501_RK   ! 2.1e1_RK
    real(RK)    , parameter :: logZoneMin = log(zoneMin)
    real(RK)    , parameter :: logZoneMax = log(zoneMax)
    real(RK)    , parameter :: DELTA_LOGZPLUS1 = (logZoneMax-logZoneMin) / (NZ-1)
    ...
    subroutine zestimate(outPath)
        ...
        do igrb = 1, NGRB
            ...
            do isample = 1, NSAMPLE
                mv_DetectionEfficiency(isample) = ParaPost(isample)%Thresh%invStdSqrt2 * ( GRB%Event(igrb)%logPF53 - ParaPost(isample)%Thresh%avg )
...
!***********************************************************************************************************************************
    ...
    pure function getProbZ(iz,igrb) result(probZ)
        ...
        use BatseSgrbWorldModel_mod, only: TIME_DILATION_EXPO
#endif
        implicit none
...
#endif
            probZ   = probZ + mv_DetectionEfficiency(isample) &
                    * exp   ( ParaPost(isample)%logNormCoef + mv_Zinfo(iz)%logSFR &
                            - 0.5_RK * dot_product( MeanSubtractedVar , matmul(ParaPost(isample)%InvCovMat,MeanSubtractedVar) ) )
